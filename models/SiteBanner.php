...
class SiteBanner extends \yii\db\ActiveRecord
{
    ...
    public function fetchAll(){
        $command = \Yii::$app->db->createCommand('SELECT * FROM site_banner');
    ...
    public function updateOne($id, $title, $content, $photo, $link_url){
        $command = \Yii::$app->db->createCommand("UPDATE site_banner SET title=:title, content=:content, photo=:photo, link_url=:link_url WHERE id=:id");
    ...
    public function insertOne($title, $content, $photo, $link_url){
        $command = \Yii::$app->db->createCommand("INSERT INTO site_banner SET title=:title, content=:content, photo=:photo, link_url=:link_url, updated_at=:updated_at");
    ...
    public function deleteOne($id){
        $command = \Yii::$app->db->createCommand("DELETE FROM site_banner WHERE id=:id");
        $command->bindValues(array(":id"=>$id));
        $res = $command->execute();
        return $res;
    }
