...
class SiteSingleBottomDetail extends \yii\db\ActiveRecord
{
    ...
    public function updateOne($id, $title, $content, $photo){
        $command = \Yii::$app->db->createCommand("UPDATE site_single_bottom_detail SET title=:title, content=:content, photo=:photo WHERE id=:id");
    ...
    public function insertOne($title, $content, $photo){
        $command = \Yii::$app->db->createCommand("INSERT INTO site_single_bottom_detail SET title=:title, content=:content, photo=:photo, updated_at=:updated_at");
