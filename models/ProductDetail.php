...
class ProductDetail extends \yii\db\ActiveRecord
{
    ...
    public function fetchAll(){
        $command = \Yii::$app->db->createCommand('SELECT * FROM product_detail');
        $res = $command->queryAll();
        return $res;
    }

    public function fetchOne($id){
        $command = \Yii::$app->db->createCommand('SELECT * FROM product_detail WHERE id=:id');
    ...
    public function updateOne($id, $title, $content, $photo, $link_name, $link_url){
        $command = \Yii::$app->db->createCommand("UPDATE product_detail SET title=:title, content=:content, photo=:photo, link_name=:link_name, link_url=:link_url WHERE id=:id");
    ...
    public function insertOne($title, $content, $photo, $link_name, $link_url){
        $command = \Yii::$app->db->createCommand("INSERT INTO product_detail SET title=:title, content=:content, photo=:photo, link_name=:link_name, link_url=:link_url, updated_at=:updated_at");
    ...
    public function deleteOne($id){
        $command = \Yii::$app->db->createCommand("DELETE FROM product_detail WHERE id=:id");
        $command->bindValues(array(":id"=>$id));
        $res = $command->execute();
        return $res;
    }
