...
class SiteSinglePic extends \yii\db\ActiveRecord
{
    ...
    public function updateOne($id, $title, $content, $photo, $link_url){
        $command = \Yii::$app->db->createCommand("UPDATE site_single_pic SET title=:title, content=:content, photo=:photo, link_url=:link_url WHERE id=:id");
    ...
    public function insertOne($title, $content, $photo, $link_url){
        $command = \Yii::$app->db->createCommand("INSERT INTO site_single_pic SET title=:title, content=:content, photo=:photo, link_url=:link_url, updated_at=:updated_at");
